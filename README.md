...
12.  Display the decrypted plaintext.
...
void generateKeyTable(char key[], int ks, char keyT[5][5]) {
    ...
    dicty['j' - 97] = 1; // 'j' is merged with 'i'

    i = 0;
    j = 0;

    // Fill key into the key table
    for (k = 0; k < ks; k++) {
        if (dicty[key[k] - 97] == 2) {
            dicty[key[k] - 97] -= 1;
            keyT[i][j] = key[k];
...
The program implementing the play fair cipher for encryption and decryption has been successfully executed, and the results have been verified.
